//! RSA-OAEP key management algorithms for JWE.
//!
//! Implements RSA-OAEP-256 (RSA with OAEP using SHA-256) and RSA-OAEP (using SHA-1).
...
/// RSA public key for encryption (RSA-OAEP-256).
///
/// This key can only encrypt, not decrypt. Use `RsaOaep256DecryptionKey` for decryption.
#[derive(Debug, Clone)]
pub struct RsaOaep256EncryptionKey {
    pk: Rsa<Public>,
    key_id: Option<String>,
}

impl RsaOaep256EncryptionKey {
    ...
    pub fn from_der(der: &[u8]) -> Result<Self, Error> {
        ...
        Ok(RsaOaep256EncryptionKey { pk, key_id: None })
    }
    ...
    /// Get the SHA-1 thumbprint of this key.
    pub fn sha1_thumbprint(&self) -> String {
        Base64UrlSafeNoPadding::encode_to_string(SHA1::hash(&self.to_der().unwrap())).unwrap()
    }

    /// Get the SHA-256 thumbprint of this key.
    pub fn sha256_thumbprint(&self) -> String {
        Base64UrlSafeNoPadding::encode_to_string(SHA256::hash(&self.to_der().unwrap())).unwrap()
    }
    ...
    pub fn encrypt_with_options<CustomClaims: Serialize>(
        ...
        let mut header = JWEHeader::new("RSA-OAEP-256", content_encryption.alg_name());
...
/// RSA key pair for decryption (RSA-OAEP-256).
///
/// This key pair can both encrypt and decrypt JWE tokens.
#[derive(Clone)]
pub struct RsaOaep256DecryptionKey {
    sk: Rsa<Private>,
    key_id: Option<String>,
}

impl std::fmt::Debug for RsaOaep256DecryptionKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RsaOaep256DecryptionKey")
...
impl RsaOaep256DecryptionKey {
    ...
    pub fn from_der(der: &[u8]) -> Result<Self, Error> {
        ...
        Ok(RsaOaep256DecryptionKey { sk, key_id: None })
    }
    ...
    pub fn encryption_key(&self) -> RsaOaep256EncryptionKey {
    ...
    pub fn decrypt_token<CustomClaims: DeserializeOwned>(
        ...
        JWEToken::decrypt("RSA-OAEP-256", token, options, |_header, encrypted_key| {
...
///
/// Note: RSA-OAEP-256 (with SHA-256) is preferred for new applications.
#[derive(Debug, Clone)]
