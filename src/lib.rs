...
-----END PUBLIC KEY-----\n";
    ...
    #[test]
    fn jwe_rsa_oaep256() {
        let decryption_key = RsaOaep256DecryptionKey::generate(2048).unwrap();
    ...
    #[test]
    fn jwe_rsa_oaep256_with_key_id() {
        let decryption_key = RsaOaep256DecryptionKey::generate(2048)
            .unwrap()
        ...
        let metadata = RsaOaep256DecryptionKey::decode_metadata(&token).unwrap();
        assert_eq!(metadata.key_id(), Some("my-key-id"));
    }
    ...
    fn jwe_rsa_key_too_small() {
        let result = RsaOaep256DecryptionKey::generate(1024);
        assert!(result.is_err());
    }
    ...
    fn jwe_rsa_malformed_inputs_no_panic() {
        let key = RsaOaep256DecryptionKey::generate(2048).unwrap();

        let malformed_inputs = [
            "",
            ".....",
            "a.b.c.d.e",
            "eyJhbGciOiJSU0EtT0FFUC0yNTYiLCJlbmMiOiJBMjU2R0NNIn0.a.b.c.d",
        ];
